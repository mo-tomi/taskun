...
### v1.2.10: 時間軸とタスク位置の正確な連動修正
**更新日**: 2025年1月10日

#### 🔧 重要な修正
- **時間軸とタスク位置の正確な連動**:
  - タスクの時間設定と縦軸の時間表示が正確に連動するように修正しました。
  - ヘッダー高さ（96px）を正確に考慮した位置計算に変更しました。
  - 6時から開始する時間軸に対して、タスクが正確な時間位置に配置されるようになりました。

#### 🎯 技術的改善
- **位置計算の精密化**: 
  - `HEADER_HEIGHT`定数を追加し、ヘッダー高さを明確に管理しました。
  - タスクの`topPosition`計算式を修正し、時間軸との正確な対応を実現しました。
  - ドラッグガイドラインの位置計算も同様に修正し、一貫性を保ちました。
- **境界値処理**: 6時より前のタスクが設定された場合の適切な処理を追加しました。

#### 🐛 解決した問題
- タスクの設定時間と縦軸の時間表示が一致しない問題
- タスクカードが時間軸の正しい位置に表示されない問題
- ドラッグ操作時のガイドラインが正確な時間位置に表示されない問題
